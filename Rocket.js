...
function handleInput() {
    const inputVector = createVector(0, 0);
    ...
    if (keyIsPressed) {
        ...
        if (keyCode === 32 || key === "x" || key === "X") {
            inputVector.set(orientation.x, orientation.y);
        }
    }

    applyThrust(inputVector);
}
...
function applyThrust(inputVector) {
    const thrustForce = createVector(inputVector.x, inputVector.y);
    thrustForce.normalize();
    thrustForce.mult(6.5); // Thrust power
